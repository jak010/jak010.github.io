...
1. **클린 아키텍처 의존성 규칙 준수**
   - 모든 소스 코드의 의존성은 항상 외부(API, DB, 인프라)에서 내부(도메인, 비즈니스 시나리오)를 향해 안쪽으로만 전파됩니다. 데이터베이스나 외부 API 어댑터 등 기술적 세부사항이 변경되더라도 핵심 비즈니스 로직은 영향을 받지 않습니다.
2. **동적 의존성 주입 (Dependency Injection)**
   - `dependency-injector` 라이브러리를 사용해 데이터베이스 세션, 타사 API 연동 어댑터 등의 인프라 의존성을 컨테이너 기반으로 관리하며, 런타임에 동적으로 주입하여 결합도를 낮추고 Mock 객체를 활용한 테스트 작성을 용이하게 만듭니다.
3. **수직적 유스케이스 분리 및 수평적 피처(Features) 배치**
   - 아티스트나 팟(Pod)과 같은 핵심 도메인 유스케이스는 수직적 레이어로 단단하게 분리하여 트랜잭션과 시나리오 일관성을 유지하고, 팔로우(follow)나 위치 정보(region) 등 재사용성과 범용성이 요구되는 보조 도메인은 `app/features/` 및 서비스 계층으로 수평 배치하여 도메인의 순수성을 해치지 않으면서도 기민하게 기능을 조립할 수 있게 하였습니다.
4. **공유 데이터 모델 및 스펙 일원화 (Shared Layer)**
   - API 응답 규격 정의를 위한 `shared/schemas`와 실제 영속성 개체인 `shared/models`를 공통 라이브러리 형태로 분리 관리하고 `Pydantic` 및 `SQLAlchemy`로 일괄 검증함으로써, 프론트엔드/모바일 클라이언트와의 소통 비용을 낮추고 중복 선언을 방어합니다.
